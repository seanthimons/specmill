...
| Catalogue discovery of Swagger `schemes` + `host` + `basePath`, relative root URLs, and OAS 3 server-variable defaults | **Supported, partially tested** | The reviewed catalogue/multi-API path resolves these through [`schema_server()`](../../../R/api-catalogue.R#L14). Direct initialization now tests root variable-default substitution; Swagger assembly and relative origin resolution still lack focused tests. Multiple distinct candidates and unresolved variables intentionally require a reviewed `base_url`. |
| Effective OAS 3 server precedence and runtime server choice | **Precedence supported; unsupported choice diagnosed** | The reader retains effective operation > path > root server metadata ([3.0 Operation Object](https://spec.openapis.org/oas/v3.0.4.html#operation-object), [3.1 Operation Object](https://spec.openapis.org/oas/v3.1.1.html#operation-object)). Because helpers still own one reviewed `BASE_URL`, path/operation overrides and multiple root choices produce `server_selection` diagnostics with exact source locations. Server-only changes are reported for review by `compare_operations()`. Covered by [`operation-server.json`](../../../tests/fixtures/capability-audit/operation-server.json) and tracked in [GH #11](https://github.com/seanthimons/specmill/issues/11). |
...
| [`operation-server.json`](../../../tests/fixtures/capability-audit/operation-server.json) | Effective operation/path/root precedence is retained, unsupported non-root selection is diagnosed at the exact source, multiple root choices are rejected, and direct initialization substitutes root variable defaults. | Partially resolved by [GH #11](https://github.com/seanthimons/specmill/issues/11) |
