...
Preview with
`generate_client(root, config = 'specmill.yml', mode = 'plan')`. The
`inventory` keeps every discovered operation, including exclusions. Its
additive `classification` field explains the disposition without
changing the existing `status` values used for selection and file
ownership:
...
Diagnostics also include `code`, `source_location` (an escaped JSON
pointer), and `guidance`. For multiple transport limitations, `reason`
lists the observed messages and the structured fields describe the first
blocker. A later metadata error takes precedence. Referenced input
locations may point into shared schema definitions. Mapped and retained
operations preserve the original classification as evidence in
`mapping_diagnostics` or `retained_diagnostics`; consult `status` for
their current generation disposition.

An unconstrained body is different from a missing body schema.
`items: {}` places no constraints on array elements. Swagger 2 body
arrays inherit Draft 4 semantics that allow omitted `items`; Swagger 2
non-body arrays and OpenAPI 3.0 array schemas require it. These contexts
receive different diagnostic codes. See [OpenAPI 3.0 Schema
Objects](https://spec.openapis.org/oas/v3.0.3.html#schema-object),
[Swagger 2
parameters](https://spec.openapis.org/oas/v2.0.html#parameter-object),
and [Draft 4 array
validation](https://json-schema.org/draft-04/draft-fge-json-schema-validation-00#rfc.section.5.3.1.4).

Do not delete an operation merely because it has no named object
properties: primitive arrays are meaningful inputs, and free-form bodies
can represent a generator limitation. Local recursion, unresolved
references, and external references now have separate codes. No
diagnostic classification establishes full schema validity or
